defmodule Rest.DnrestApi do
  use Maru.Server, otp_app: :elixir_dns_rest_server
end
  ...
  namespace :dns do
    namespace :srv do
      ...
      delete do
        ...
        Logger.debug(fn ->
          Enum.map_join(val_map, ", ", fn {key, val} -> ~s{#{key}: #{val}} end)
        end)
        ...
        case params.ip_address |> DNS.IP_Utils.to_ipv4() do
          {:ok, address} ->
            DNS.Cache.set_record(
              :a,
              params.host |> to_string |> DNS.Cache.canonicalize(),
              address
            )
            ...
            |> put_status(200)
            |> json(%{
              key: DNS.Cache.canonicalize(params.host),
              record: to_string(:inet.ntoa(address))
            })
